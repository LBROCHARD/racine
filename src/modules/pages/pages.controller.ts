import { Body, Controller, Post, UseGuards, Request, HttpException, HttpStatus, Get, Put, Delete, Param } from '@nestjs/common';
...
export class PagesController {
    ...
    @UseGuards(AuthGuard)
    @Get("from-group/:id")
    getPagesFromGroupId(@Request() req: AuthenticatedRequest, @Param('id') groupId: string,) {
        return this.pagesService.getAllPagesFromGroupId(req.user.id, groupId);
    }

    @UseGuards(AuthGuard)
    @Get("/:id")
    getPagesFromPageId(@Request() req: AuthenticatedRequest, @Param('id') pageId: string,) {
    ...
    async updatePageContent(
        ...
        try {
            return this.pagesService.updatePageContent(req.user.id, updatePageContentDto);
        } catch (error) {
            if (error instanceof HttpException) {
            throw error;
            }
